 ...
 */
public class MentionsTimelineFragment extends FeedFragment {
    @Override
    public void retrieveTweets(final boolean clearOld) {
        getTwitterClient().getMentions(lastTweetId, TWEETS_PER_PAGE, new TwitterClient.Callback() {
            @Override
            ...
            public void onFailure(int statusCode, String error, Throwable t) {
                Log.e(MentionsTimelineFragment.class.getSimpleName(), "Failed to get feed. " + error, t);
                Toast.makeText(MentionsTimelineFragment.this.getContext(), "Network error during feed load", Toast.LENGTH_LONG).show();
                swipeContainer.setRefreshing(false);
            }
